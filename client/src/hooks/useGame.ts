...
export function getLevelConfig(level: number): LevelConfig {
  if (level <= 3) return { wordCount: 50, targetWords: 1, flashSpeed: 800 };
  if (level <= 6) return { wordCount: 60, targetWords: level <= 4 ? 1 : 2, flashSpeed: 650 };
  if (level <= 9) return { wordCount: 70, targetWords: 2, flashSpeed: 500 };
  if (level <= 12) return { wordCount: 80, targetWords: 2, flashSpeed: 400 };
  if (level <= 15) return { wordCount: 90, targetWords: level <= 13 ? 2 : 3, flashSpeed: 300 };
  return { wordCount: 100, targetWords: 3, flashSpeed: 250 };
}
